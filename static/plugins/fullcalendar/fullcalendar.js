...
var RowRenderer = Class.extend({
	...
	// Also applies the "intro" and "outro" cells, which are specified by the subclass and admin.
...
function compareForwardSlotSegs(seg1, seg2) {
    ...
    /* An abstract class from which other admin inherit from
----------------------------------------------------------------------------------------------------------------------*/
...
var View = fc.View = Class.extend({
	...
	function getViewSpec(requestedViewType) {
		...
		var hash = options.views || {}; // the `admin` option object
		...
		while (viewType && !viewClass) {
			...
			processSpecInput(fcViews[viewType]); // $.fullCalendar.admin, lower precedence
	...
	function zoomTo(newDate, viewType) {
		...
		if (!viewType || !isValidViewType(viewType)) { // a general view name, or "auto"
			...
			viewStr = header.getViewsWithButtons().join(' '); // space-separated string of all the admin in the header
...
function backupEventDates(event) {
    ...
    /* An abstract class for the "basic" admin, as well as month view. Renders one or more rows of day cells.
----------------------------------------------------------------------------------------------------------------------*/
...
var BasicView = fcViews.basic = View.extend({
	...
	computeRange: function(date) {
		...
		// year and month admin should be aligned with weeks. this is already done for week
...
fcViews.basicDay = {
    ...
    /* An abstract class for all agenda-related admin. Displays one more columns with time slots running vertically.
----------------------------------------------------------------------------------------------------------------------*/
